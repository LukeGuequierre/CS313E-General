...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        while i < len(guessed_word):
            color = self.colors[guessed_word[i]]
            if color != CORRECT_COLOR:
                if color != WRONG_SPOT_COLOR or feedback_colors[i] == CORRECT_COLOR:
                    self.colors[guessed_word[i]] = feedback_colors[i]
            i += 1
    ...
    def __str__(self):
        ...
        for letter in self.rows[0]:
            ...
            if self.rows[0].index(letter) + 1 == len(self.rows[0]):
                str_keyboard += colored_letter
            else:
                str_keyboard += colored_letter + ' '
        str_keyboard += '\n '
        for letter in self.rows[1]:
            colored_letter = color_word(self.colors[letter], letter)
            if self.rows[1].index(letter) + 1 == len(self.rows[1]):
                str_keyboard += colored_letter
            else:
                str_keyboard += colored_letter + ' '
        str_keyboard += '\n   '
        for letter in self.rows[2]:
            colored_letter = color_word(self.colors[letter], letter)
            if self.rows[2].index(letter) + 1 == len(self.rows[2]):
                str_keyboard += colored_letter
            else:
                str_keyboard += colored_letter + ' '
        return str_keyboard
...
class WordFamily:
    ...
    def __lt__(self, other):
        ...
        if self.difficulty < other.difficulty:
            return False
        self_string = ''.join(self.feedback_colors)
        other_string = ''.join(other.feedback_colors)
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for i in range(NUM_LETTERS):
        ...
        elif l == secret_word[i]:
            feedback[i] = CORRECT_COLOR
        else:
            if g_dict[l] <= s_dict[l]:
                feedback[i] = WRONG_SPOT_COLOR
            else:
                wrongspot = False
                for n in range(i+1, NUM_LETTERS):
                    if guessed_word[n] == l and secret_word[n] != guessed_word[n]:
                        wrongspot = True
                if wrongspot:
                    feedback[i] = WRONG_SPOT_COLOR
                else:
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    for word in remaining_secret_words:
        ...
        if not word_families:
            words = [word]
            new_family = WordFamily(feedback_colors, words)
            word_families.append(new_family)
        else:
            present = False
            for family in word_families:
                if isinstance(family, WordFamily):
                    if family.feedback_colors == feedback_colors:
                        present = True
                        family.words.append(word)
            if not present:
                words = [word]
                new_family = WordFamily(feedback_colors, words)
                word_families.append(new_family)
